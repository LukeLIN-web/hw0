...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    reverse_topo_order = list(reversed(find_topo_sort([output_tensor]))) # 但是这样的话不还是从输入到输出算梯度吗?

    ### BEGIN YOUR SOLUTION

    for node in reverse_topo_order:
        node.grad = sum(node_to_output_grads_list[node])
        if node.is_leaf(): # 不加括号编译可以通过
            continue
        for i, grad in enumerate( node.op.gradient_as_tuple(node.grad, node)):
            ...
            node_to_output_grads_list[k].append(grad) # 为什么grad * node.op.gradient(node, k)不对?  因为把操作符也作为node,已经计算过了
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, visited, topo_order):
    """Post-order DFS ,因为只有node, 没有所有边的信息,所以要dfs"""
    ### BEGIN YOUR SOLUTION
    if node in visited:
        return
